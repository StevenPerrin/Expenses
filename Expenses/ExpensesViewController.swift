...
class ExpensesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if let date = dateformatter.date(from: "May 30, 2018 11:43"){
            expenses.append(Expense(title: "Uber", amount: 32.50, date: date))
        }
        if let date = dateformatter.date(from: "May 29, 2018 8:45"){
            expenses.append(Expense(title: "Coffee", amount: 32.50, date: date))
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = cell as? ExpenseTableViewCell{
            ...
            cell.dateLabel.text = displayDateFormatter.string(for: expense.date)
